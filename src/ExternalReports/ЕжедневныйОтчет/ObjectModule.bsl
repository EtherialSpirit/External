...
Перем мИмяНастройки Экспорт;


Функция СведенияОВнешнейОбработке() Экспорт
	ПараметрыРегистрации = Новый Структура;
	ИмяОтчета = "Ежедневный отчет";
	ПараметрыРегистрации.Вставить("Вид", "ДополнительныйОтчет");
    ПараметрыРегистрации.Вставить("Наименование", ИмяОтчета);
    ПараметрыРегистрации.Вставить("Версия", "v1.1");
	ПараметрыРегистрации.Вставить("БезопасныйРежим", Ложь);
	ПараметрыРегистрации.Вставить("Информация", ИмяОтчета);
	ТаблицаКоманд = ПолучитьТаблицуКоманд();
	ДобавитьКоманду(ТаблицаКоманд, ИмяОтчета, "2", "ОткрытиеФормы", Истина);
...
Функция ПолучитьТекстЗаголовкаОтчета() Экспорт
	...
	Если НайденнаяНастройка.Использование Тогда
		...
		мНазваниеОтчета = ЗаголовокОтчета;
	КонецЕсли;
...
// Устанавливает заголовок формы отчета в зависимости от настроек отчета
//
Процедура УстановитьЗаголовокФормы(Отчет, Форма, ИмяВарианта = Неопределено) Экспорт
	
	Форма.Заголовок = ПолучитьТекстЗаголовкаОтчета()+", настройка = " + мИмяНастройки;
	
	мНазваниеОтчета = Форма.Заголовок;
...
// Определяет возможность вывода отчета
//
Функция ВыводВозможен()
	// сюда необходимо вставить условия, запрещающий вывод отчета ...
	Возврат Истина;
КонецФункции	
