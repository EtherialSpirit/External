...
&НаКлиенте
Процедура ТД(Команда)
	
	ТабДокумент = Новый  ТабличныйДокумент;
	
	ТабДокумент = ПечатьНаСервере(Объект.СсылкаНаДокумент, ТабДокумент);
	
	ТабДокумент.Показать("Печать документа");
	
КонецПроцедуры

&НаСервере
Функция ПечатьНаСервере(СсылкаНаДокумент, ТабДокумент)
	
	Макет = РеквизитФормыВЗначение("Объект").ПолучитьМакет("ЗаказНаряд");
