...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Наименование", "ЗаказНаряд"); //имя под которым обработка будет зарегестрирована в справочнике внешних обработок
	...
	ДобавитьКоманду(ТаблицаКоманд, "ЗаказНаряд", "ЗаказНаряд", "ВызовСерверногоМетода", Истина, "ПечатьMXL");
...
Процедура Печать(МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ЗаказНаряд", "ЗаказНаряд", СформироватьПечатнуюФорму(МассивОбъектов[0], ОбъектыПечати));
...
Функция СформироватьПечатнуюФорму(СсылкаНаДокумент, ОбъектыПечати) Экспорт
	...
	ТабДокумент.ИмяПараметровПечати = "ЗаказНаряд";
	Макет                           = ПолучитьМакет("ЗаказНаряд");
	
	ТабДокумент.ОриентацияСтраницы=ОриентацияСтраницы.Портрет;
	ТабДокумент.АвтоМасштаб= истина;
	ТабДокумент.ПолеСверху = 5; //Поле сверху 25мм,
	ТабДокумент.ПолеСлева=15;//Поле слева 25мм, 
	
	ОбластьШапка = Макет.ПолучитьОбласть("Шапка");	                                                                           
	ОбластьШапка.Параметры.НомерДок   = СсылкаНаДокумент.Номер;
	ОбластьШапка.Параметры.Заказчик   = СсылкаНаДокумент.Контрагент;
	ОбластьШапка.Параметры.ДатаПриема =	Формат(СсылкаНаДокумент.Дата, "ДЛФ=D"); 
	ОбластьШапка.Параметры.ДатаСдачи  = Формат(СсылкаНаДокумент.Дата, "ДЛФ=D");	
	ТабДокумент.Вывести(ОбластьШапка);
	
	ИтогоУслуг    = 0;
	ИтогоЗапчасти = 0;
	
	Для Каждого Строка Из СсылкаНаДокумент.Товары Цикл
		
		Если Строка.Номенклатура.ВидНоменклатуры = Справочники.ВидыНоменклатуры.НайтиПоНаименованию("Услуги") Тогда
			СтрокаУслуги = Макет.ПолучитьОбласть("СтрокаУслуги");
			СтрокаУслуги.Параметры.Услуги     = Строка.Номенклатура;
			СтрокаУслуги.Параметры.КолвоУслуг = Строка.Количество;
			СтрокаУслуги.Параметры.ЦенаУслуг  = Строка.Цена;	
			СтрокаУслуги.Параметры.СуммаУслуг = Строка.Сумма;
			СтрокаУслуги.Параметры.Исполнитель= СсылкаНаДокумент.Менеджер;
			ИтогоУслуг = ИтогоУслуг + Строка.Сумма;
			ТабДокумент.Вывести(СтрокаУслуги);
		Иначе 
			СтрокаЗапчасти = Макет.ПолучитьОбласть("СтрокаЗапчасти");
			СтрокаЗапчасти.Параметры.Запчасти	   = Строка.Номенклатура;			
			СтрокаЗапчасти.Параметры.Артикул	   = Строка.Номенклатура.Артикул;	
			СтрокаЗапчасти.Параметры.ЕдИзм	       = Строка.Номенклатура.ЕдиницаИзмерения;
			СтрокаЗапчасти.Параметры.КолвоЗапчасти = Строка.Количество;
			СтрокаЗапчасти.Параметры.ЦенаЗапчасти  = Строка.Цена;	
			СтрокаЗапчасти.Параметры.СуммаЗапчасти = Строка.Сумма;
			ИтогоЗапчасти = ИтогоЗапчасти + Строка.Сумма;
			ТабДокумент.Вывести(СтрокаЗапчасти);
		КонецЕсли; 
		
	КонецЦикла; 	
	
	ОбластьШапкаЗ = Макет.ПолучитьОбласть("ШапкаЗапчасти"); 	                                                                           
	ОбластьШапкаЗ.Параметры.ИтогоСуммаУслуг = ИтогоУслуг;		
	ТабДокумент.Вывести(ОбластьШапкаЗ);	
	
	ОбластьПодвал = Макет.ПолучитьОбласть("Подвал");   	                                                                           
	ОбластьПодвал.Параметры.ИтогоСуммаЗапчасти = ИтогоЗапчасти;	
	ОбластьПодвал.Параметры.Исполнитель        = СсылкаНаДокумент.Менеджер;
	ОбластьПодвал.Параметры.ИтогоПоДокументу   = ИтогоУслуг + ИтогоЗапчасти;
